...
public  class QueryUserURL{
	
	StringBuffer res = null;
	public QueryUserURL(String sid, String from, String to)throws Exception{
		String url = "http://hyldeb.ddns.net/~hyl/youbike/query_user_operations.php";
		URL obj = new URL(url);
		HttpURLConnection conn = (HttpURLConnection) obj.openConnection();
	
		conn.setRequestMethod("POST");
	
		String Parameter = "id="+sid+"&from="+from+"&to="+to;
		...
		try{
			...
			try{
				for(int i = 0 ; i < date_count ; i++){
					operation[i] = json_arr.getJSONObject(i).getJSONArray("operations");
					date[i] = json_arr.getJSONObject(i).getString("date");
					each_operation_size[i] = operation[i].length();
					total_line+=each_operation_size[i]*6;
				}
				...
				for(int i = 0 ; i<date_count ; i++){
					...
					for(int j = 0 ; j<each_operation_size[i] ; j++){
						...
						msg.append("  Station ID¡G"+operation[i].getJSONObject(j).getInt("id")+"\n");
						int action = operation[i].getJSONObject(j).getInt("operation");
						if(action==0){
							msg.append("  Action     ¡GRent\n");
						}
						else {
							msg.append("  Action     ¡GReturn\n");
							msg.append("  Duration  ¡G" + operation[i].getJSONObject(j).getInt("duration")+" seconds\n");
							msg.append("  Charge   ¡G" + operation[i].getJSONObject(j).getInt("charge")+"\n");
						}
